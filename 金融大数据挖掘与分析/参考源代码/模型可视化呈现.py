...
# # 二、模型可视化呈现 对于决策树模型可视化感兴趣的可以参考以下网址：
# https://shimo.im/docs/lUYMJX0TEjoncFZk
...
with codecs.open(txt_dir, 'r') as f, codecs.open(txt_dir_utf8, 'w', encoding='utf-8') as wf:
    for line in f:
        ...
        wf.write(newline + '\t')
wf.close()
os.system('dot -Tpng dot_data_utf8.txt -o example_new.png')  # 以PNG的图片形式存储生成的可视化文件
print('可视化文件example.png已经保存在代码所在文件夹！')

os.system('dot -Tpdf dot_data_utf8.txt -o example_new.pdf')  # 以PDF的形式存储生成的可视化文件
